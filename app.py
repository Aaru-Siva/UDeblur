...
def main():
    ...
    opt['dist'] = False
    UDeblur = create_model(opt)

    # Create the Streamlit app
    st.title('UDeblur - Image Deblurring App')

    # Create a list of items
    items = ["Upload blur image below", "You can upload images in PNG, JPG, JPEG, BMP, WEBP format",
             "You can able to deblur multiple images one after another without going back"]
                ...
                if uploaded_files:
                    for uploaded_file in uploaded_files:
                        ...
                        output_path = os.path.join(
                            result_folder, uploaded_file.name)
                        single_image_inference(UDeblur, inp, output_path)

                    # Display the input and output images
                    st.success(
                        'Congratulations! You have successfully completed the operation.')
